...
export default function App() {
  ...
  const generateStrategy = async () => {
    ...
    try {
      ...
      const prompt = `Aja como o Diretor de Cria├º├úo e Estrat├®gia de Elite da MASTER FUNIL MARKETING MAIS CORPORATIVO. 
Crie um ecossistema de marketing digital de ALTA PERFORMANCE e COMPLETAMENTE DETALHADO para "${negocio}".

Ideia Central: ${ideia}
P├║blico-alvo: ${publico}
Estilo de Comunica├º├úo: ${estilo} (Siga rigorosamente este estilo)
Formatos Solicitados: ${formatos.join(', ')}
${formatos.some(f => f.includes('Carrossel')) ? `- Slides por Carrossel: ${slidesCarrossel}` : ''}

ESTRUTURA OBRIGAT├ôRIA DA RESPOSTA (N├âO PULE NENHUMA SE├ç├âO):

1. POSICIONAMENTO DE ELITE E PROPOSTA DE VALOR
   - Defina o tom de voz e a autoridade da marca.
   - Proposta ├Ünica de Valor (UVP) focada em High-End Corporate.

2. ARQUITETURA DO FUNIL DE CONVERS├âO (DETALHADO)
   - TOPO DE FUNIL (ToFu): Atra├º├úo e consci├¬ncia.
   - MEIO DE FUNIL (MoFu): Desejo e qualifica├º├úo.
   - FUNDO DE FUNIL (BoFu): Escassez e convers├úo direta.

3. MIX DE CANAIS ESTRAT├ëGICOS & PLANO DE TR├üFEGO PAGO
   - Detalhe estrat├®gias para LinkedIn Ads, Meta Ads e Google Search.
   - Distribui├º├úo de verba e foco de ROI.

4. CRONOGRAMA DE IMPACTO (PLANO DE A├ç├âO DE 30 DIAS)
   - Divida em fases: Teaser, Line-up, Tra├º├úo e Escassez.

5. ROTEIRO DE NARRA├ç├âO PROFISSIONAL (MASTER)
   - Texto completo para locu├º├úo em PT-BR, com tom grave, compassado e de autoridade m├íxima.

REQUISITOS T├ëCNICOS ADICIONAIS:
1. IDIOMA: Todo o conte├║do (texto, labels, scripts e QUALQUER TEXTO DENTRO DAS IMAGENS) deve ser em PORTUGU├èS DO BRASIL (PT-BR) de alt├¡ssimo n├¡vel.
2. ASSETS_PROMPTS: No final, adicione a se├º├úo "ASSETS_PROMPTS" com prompts cinematogr├íficos para CADA formato solicitado.
3. REGRAS PARA IMAGENS:
   - Cada asset deve ser identificado como: [ASSET: Nome do Formato (Indique a Propor├º├úo 1:1 ou 9:16 ou 16:9 aqui) | PROMPT: Descri├º├úo em ingl├¬s, cinematogr├ífica, focada no produto. MANDATORILY: Any text appearing inside the image MUST be in Brazilian Portuguese (PT-BR)].
   - Para Carross├®is, gere prompts para os ${slidesCarrossel} slides: [ASSET: Nome do Carrossel - Slide X (Propor├º├úo) | PROMPT: ...].
4. V├ìDEO E ├üUDIO:
   - [VIDEO_PROMPT: Descri├º├úo cinematogr├ífica em ingl├¬s para v├¡deo de impacto de 8s. Any visible text MUST be in PT-BR].
   - [NARRATION_SCRIPT: Texto persuasivo em PT-BR para locu├º├úo de 8s].

IMPORTANTE: O texto deve ser extenso, denso, focado em convers├úo e autoridade absoluta. N├úo seja sucinto. Seja completo e profissional.`;
      ...
      const textResponse = await fetch('/api/ai/generate-text', {
        ...
        body: JSON.stringify({
          prompt: prompt,
          systemInstruction: "Voc├¬ ├® o Diretor de Cria├º├úo da MASTER FUNNEL. Sua miss├úo ├® entregar estrat├®gias de marketing de elite, com portugu├¬s impec├ível (PT-BR), tom persuasivo e autoridade absoluta.",
          model: textModel
        })
      });
      ...
      if (err.status === 404) {
        ...
        setError("N├úo Autorizado: Clique no bot├úo 'Configurar Chave de API' e selecione uma chave v├ílida (necess├írio para V├¡deo/Imagem).");
      } else {
  ...
  const generateVideoAsset = async () => {
    ...
    try {
      // 1. Inicia a Gera├º├úo no Backend (Vertex AI)
      ...
      setStatus('Renderizando v├¡deo cinematogr├ífico (Veo 3.1)...');
      ...
      while (!isDone && attempts < maxAttempts) {
        ...
        // Progresso visual mais r├ípido para o modelo FAST
        ...
        if (statusData.done) {
          ...
          if (statusData.videoUri) {
            ...
            // Define o v├¡deo usando o endpoint de proxy para estabilidade e CORS
            setGeneratedVideo(`/api/ai/video-proxy?url=${encodeURIComponent(statusData.videoUri)}`);
            setStatus('V├¡deo gerado com sucesso!');
          } else if (statusData.error) {
            throw new Error(`IA Error: ${statusData.error.message || 'Falha na renderiza├º├úo'}`);
          }
        }
      }
      ...
      setVideoError(err.message || 'Erro desconhecido ao processar o v├¡deo');
    } finally {
  ...
  const generateAudio = async () => {
    ...
    try {
      ...
      if (data.data) {
        ...
        throw new Error("O servidor n├úo retornou dados de ├íudio.");
      }
    } catch (err: any) {
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                          <div className="flex-1 space-y-4">
                            ...
                            <div className="flex flex-col gap-3 py-2">
                              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#f58f2a]">Formato do V├¡deo</span>
                              <div className="flex items-center gap-4">
                                <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
                                  <button
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-white/40">
                                        <span>Progresso da Gera├º├úo</span>
